...
class GoogleAnalyticsAPI
{
	...
	public function getMetric($metric,$uri=null)
	{
		...
		if($this->getContent($url) == 200) {
			...
			if($XML_object !== false){ //Correction Warning: get_object_vars() [function.get-object-vars]: Node no longer exists in D:\EasyPHP\www\ecommerce.makitizi.com\cake\libs\debugger.php on line 432 - 18/03/2011 by AB
				$sCheckEntry = $XML_object->entry; //Correction Warning: get_object_vars() [function.get-object-vars] (suite)
				
				if (@count($sCheckEntry)>0) {
					
					$dxp = $XML_object->entry->children('http://schemas.google.com/analytics/2009');
					
					$metric_att = array(); // Correction Notice (8): Undefined variable: metric_att [APP\vendors\analytics\api_ga.php, line 286] - 18/03/2011 by AB
					if (@count($dxp)>0) {
						$metric_att = $dxp->metric->attributes();
					}
 ...
 * 
 * fonction qui permet d'ontenir le jeton de connexion afin de verifier que la connexion à Google Analytics est bien réalisée
 *
 * @return valeur du jeton de connexion
 * @access public
 * @author Az
 * @version 0.1 - 16/03/2011 by AB
 */
