 ...
 */		
	function init_db($db_host, $db_name, $db_username, $db_password, $file, $start, $foffset, $totalqueries) {
		...
		if(!$dbconnection) $aErrors[] = "Database connection failed due to ".utf8_encode(mysqli_error($dbconnection));

		//set charset
		if(count($aErrors) == 0 && $db_connection_charset !== '') @mysqli_query($dbconnection, "SET NAMES $db_connection_charset", $dbconnection);
		...
		if(count($aErrors) == 0 && isset($start) && isset($foffset) && preg_match("/(\.(sql|gz|csv))$/i", $curfilename)) {
				...
				while($linenumber < $start + $linespersession || $query != "") {
					...
					if(preg_match('/'.preg_quote($delimiter).'$/',trim($dumpline)) && !$inparents) { 
						...
						if(!mysqli_query($dbconnection, $query)) { 
							...
							$aErrors[] = array(
								'message' => "Error at the line $linenumber: ". trim($dumpline)." - MySQL: ".mysqli_error($dbconnection),
			...
			if(count($aErrors) == 0) { 
				...
				if(!$gzipmode) {
					...
					$pct_bar  = '<div class="progress progress-xs progress-striped active"><div class="progress-bar progress-bar-primary" style="width: 100%"></div></div>';
				} else {
