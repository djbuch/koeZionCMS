...
class CKFinder_Connector_CommandHandler_Pixlr extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    ...
    {  
		...
		$pixlrSession = array(
			'token' => substr($maketoken,0,16),
			'ImagePath' => $filePath,
			'clientImagePath' => $this->_currentFolder->getUrl(),
			'fileName' => $fileName,
			'return' => $_SERVER['HTTP_REFERER'],
			'thumbLocation' => $thumbFolder . $fileName
		);
		...
		$pixlr_params = array("referrer" => $_SERVER['HTTP_HOST'],
								  ...
								  "target" => urlencode($clientPluginFolder."/process.php?token=".$pixlrSession['token']),
								  "locktarget" => "TRUE",
								  "locktitle" => "TRUE",
								  "locktype" => "TRUE",
								  "lockquality" => "80"
