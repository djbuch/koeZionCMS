 ...
 */	
	function create_form($formulaire, $formInfos) {
		...
		foreach($formulaire as $numeroQuestion => $question) {
			...
			switch($question['type']) {
				case 'select':
					...
					echo $this->input($inputName, $sTxtConstraint.$question['label'], $params);
				...
				case 'checkbox':							
					...
					$donneesPostees = $this->view->controller->request->data;
					...
					foreach($inputValues as $value => $sLibelle) {							
						...
						$inputNameText = $this->_set_input_name($aInputName[$cptName]); //Mise en variable du name de l'input
						$inputIdText = $this->_set_input_id($inputNameText); //Mise en variable de l'id de l'input
				...
				case 'radio':
					...
					$inputName = $question['name'][0];
					$inputNameText = $this->_set_input_name($inputName); //Mise en variable du name de l'input
					$inputIdText = $this->_set_input_id($inputNameText); //Mise en variable de l'id de l'input
				...
				case 'hidden':
					...
					$inputName = $question['name'][0];
					echo $this->input($inputName, '', array('type' => 'hidden', 'value' => $question['default']));
